...
const InputField = ({ label, type, id, name }) => (
  <div className="mb-6">
    <label htmlFor={id} className="block text-2xl font-medium text-gray-700 mb-2">{label}</label>
    <input
      type={type}
      id={id}
      name={name}
    ...
    <div className="min-h-screen flex items-center justify-center font-bubblegum bg-white p-4">
      <div className="bg-white p-10 rounded-3xl shadow-2xl w-full max-w-4xl border-8 border-primary">
        ...
        <form className="space-y-6">
          {!isLogin && (
            <>
            <InputField label="Your Awesome Name" type="text" id="name" name="name" />
            <InputField label="Your Phone number" type="text" id="number" name="number" />
            <InputField label="Your Record ID" type="text" id="record" name="record" />
            <InputField label="Your Awesome Picture" type="file" id="picture" name="picture" />
            </>
          )}
          <InputField label="Magic Email" type="email" id="email" name="email" />
          <InputField label="Super Secret Code" type="password" id="password" name="password" />
        ...
        {isLogin && (
          <div className="mt-6">
            ...
            <div className="mt-6">
              <button onClick={() => window.open("http://localhost:3000/auth/google")}
                type="button"
              ...
              <InputField label="Your record ID for google login" type="text" id="number" name="number" />
        ...
        <p className="mt-8 text-center text-xl text-gray-600">
          {isLogin ? "New to our world? Come play with us!" : "Already part of our club?"}
          <button
            onClick={toggleForm}
